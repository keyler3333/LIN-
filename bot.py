...
def sandbox_real_lua(source, timeout=8):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        i = 1
        while True:
            pth = os.path.join(tmpdir, f'layer_{i}.lua')
...
async def deobf(ctx, flags: str = ''):
    ...
    final, obf_type, layers, constants = await asyncio.to_thread(deobfuscate_engine, text)
    if '--ai' in flags and ANTHROPIC_KEY:
        final = await ai_clean(final)
    ...
    file = discord.File(fp=io.StringIO(final), filename=f'deobf_{att.filename}')
