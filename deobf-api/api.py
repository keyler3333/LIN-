...
def run_sandbox(source, timeout=25):
    with tempfile.TemporaryDirectory() as d:
        ...
        esc_inp = in_path.replace('\\', '\\\\').replace('"', '\\"')
        driver  = runtime\
            .replace('OUTDIR_PLACEHOLDER', esc_dir)\
            .replace('INPATH_PLACEHOLDER', esc_inp)
...
def deobfuscate(source):
    ...
    if layers > 0:
        ...
        result = beautify(result)
        note = ''
        if obf_type in ('ironbrew2',):
            note = ' IronBrew2 requires C#-level deobfuscation for full reversal.'
        return result, obf_type, 0, 'static', diag + note
