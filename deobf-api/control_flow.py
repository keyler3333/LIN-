...
def detect_state_machine(block):
    ...
    for stmt in block.statements:
        if isinstance(stmt, While) and isinstance(stmt.test, Boolean) and stmt.test.value == True:
            pattern_while_true = True
            inner = flatten_to_structured(stmt.body.statements)
...
def reconstruct_control_flow(block):
    ...
    for stmt in block.statements:
        if isinstance(stmt, While) and isinstance(stmt.test, Boolean) and stmt.test.value == True:
            inner_stmts = flatten_to_structured(stmt.body.statements)
            if not inner_stmts:
                continue
            state_var = None
            cases = []
            ...
            if state_var and len(cases) > 1:
                reconstructed = _build_switch(state_var, cases, fallback)
                new_stmts.append(reconstructed)
                continue
