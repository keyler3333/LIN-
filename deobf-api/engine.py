...
class DeobfEngine:
    ...
    def process(self, source):
        ...
        diags = [c for c in all_captures if isinstance(c, str) and any(c.startswith(p) for p in _DIAG_PREFIXES)]
        ...
        if ranked:
            ...
            if isinstance(best, str):
                ...
                if score > 0:
                    return self._beautify(best), method, f'Score: {score}'
                ...
                return self._beautify(best), 'fallback', 'Returning best available'

        diag_msg = diags[0] if diags else 'No candidates produced'
        return self._beautify(static_out if changed else source), 'fallback', diag_msg
