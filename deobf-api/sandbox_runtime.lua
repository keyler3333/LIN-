...
rawset = function(t,k,v) _track_string(v); return _orig_rawset(t,k,v) end
rawget = function(t,k) return _orig_rawget(t,k) end
table.concat = function(t,sep,i,j) local r=_orig_table_concat(t,sep,i,j); _track_string(r); return r end
string.char = function(...) local r=_orig_string_char(...); _track_string(r); return r end
...
local function _make_service(n) return _roproxy() end

local Players = {
    LocalPlayer = {
        Name="Player", UserId=1,
        Character=_roproxy(), PlayerGui=_roproxy(), Backpack=_roproxy(),
        PlayerScripts=_roproxy(), Team=nil, membershipType=0
    },
...
local Instance = {
    new=function(className)
        local obj=_roproxy()
        obj.Name=className; obj.className=className; obj.Parent=nil
        obj.Destroy=function() end; obj.Clone=function() return obj end
...
local CFrame = {
    new=function(...)
        local args={...}
        if #args==3 then return {x=args[1],y=args[2],z=args[3],lookVector=Vector3.new(0,0,1)}
        elseif #args==12 then return {} end; return {}
    end,
...
local _safe_env = {
    assert=assert, error=function(msg,level) if msg=="detected by LeakD" then return nil end; error(msg,level or 0) end,
    ipairs=ipairs, next=next, pairs=pairs, pcall=pcall, rawequal=rawequal,
    rawget=rawget, rawlen=rawlen, rawset=rawset, select=select,
    ...
    table={ concat=table.concat,insert=table.insert,maxn=function(t) local n=0; for k in pairs(t) do if type(k)=="number" and k>n then n=k end end; return n end, remove=table.remove,sort=table.sort,unpack=_orig_unpack or table.unpack },
    ...
    game=game, workspace=_roproxy(), Players=Players,
    MarketplaceService=_make_service("MarketplaceService"),
    ReplicatedStorage=_roproxy(), ServerStorage=_roproxy(),
    ServerScriptService=_roproxy(), Lighting=_roproxy(),
    StarterGui=_roproxy(), StarterPack=_roproxy(),
...
local _env_mt = {
    ...
    __newindex = function(t,k,v) _track_string(v); _orig_rawset(_safe_env,k,v) end
}
local env = setmetatable({}, _env_mt)

local fh = _io.open(_inp, "rb")
if not fh then
    _L("OPEN_ERROR"); _write_file(_out.."/error.txt", "cannot open input")
else
    local source = fh:read("*a"); fh:close()
    if not source then
        _L("READ_ERROR"); _write_file(_out.."/error.txt", "cannot read input")
    else
        _L("SIZE: "..#source.." bytes")
        local chunk, parse_err = _orig_loadstring(source, "@input")
        if not chunk then
            _L("PARSE: "..tostring(parse_err)); _write_file(_out.."/error.txt", "parse error: "..tostring(parse_err))
        else
            _orig_setfenv(chunk, env)
            local function error_handler(e)
                local tb = debug.traceback(_orig_tostring(e),2)
                _L("TRACE: "..tb); return tb
            end
            local ok, res = _orig_xpcall(chunk, error_handler)
            if not ok then
                _L("RUNTIME: ".._orig_tostring(res)); _write_file(_out.."/error.txt", _orig_tostring(res))
            else
                _L("DONE: ".._orig_type(res))
                if res and _orig_type(res)=="function" then
                    local ok2, bc = pcall(string.dump, res)
                    if ok2 then _write_file(_out.."/dump.bin", bc, "wb"); _L("DUMPED "..#bc.." bytes") end
                elseif res and _orig_type(res)=="string" and #res>5 then
                    _write_layer(res); _L("RETURNED "..#res.." bytes")
                end
            end
            ...
            local function _scan_table(t, depth, path)
                ...
                for k,v in _orig_pairs(t) do
                    if _orig_type(v)=="string" and #v>5 then
                        _track_string(v); mem_parts[#mem_parts+1]=v
            ...
            if #mem_parts>0 then
                local mf = _io.open(_out.."/memory.txt", "w")
                if mf then
