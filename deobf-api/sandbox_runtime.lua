...
rawget = function(t, k)
    ...
    if v == nil and _env_tables[t] then
        v = _roproxy()
        _orig_rawset(t, k, v)
    end
    return v
...
local game = {
    ...
    GetService = function(self, name)
        ...
        else return _make_service(name) end
    end,
...
local _safe_env = {
    ...
    ipairs = ipairs, next = next, pairs = pairs, pcall = pcall,
...
_env_tables[_safe_env] = true

setmetatable(_safe_env, {
    __index = function(t, k)
        local p = _roproxy()
        _orig_rawset(t, k, p)
        return p
...
for i, bc in ipairs(found_bytecode) do
    local escaped = bc:gsub(".", function(c)
