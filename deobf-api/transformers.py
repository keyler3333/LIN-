    ...
    def _func(self, func, name='f', is_main=False):
        ...
        while i < len(code):
            ...
            if op == 0: regs[a] = R(b)
                ...
                regs[a] = gn[1:-1] if gn.startswith(('"', "'")) else gn
...
class WeAreDevsLifter(Transformer):
    ...
    def _try_lift(self, source):
        ...
        if not decoded:
            self.diagnostic = "No strings decoded successfully"
            return None
        ...
        if self._is_lua51_bytecode(full):
            return self._decompile_bytecode(full)
        ...
        if best: return best
        self.diagnostic = f"Decoded {len(decoded)} chunk(s), {len(full)} bytes - not valid Lua"
        return None
    ...
    def _unescape_lua_string(s):
        ...
        while i < len(s):
            if s[i] == '\\' and i+1 < len(s):
                ...
                if nc.isdigit():
                    j = i+1
                    while j < len(s) and s[j].isdigit() and j-i <= 3: j += 1
                    digits = s[i+1:j]
                    if digits:
                        val = int(digits)
                        if val < 256: result.append(chr(val)); i = j; continue
                elif nc == 'x' and i+3 < len(s):
                    hd = s[i+2:i+4]
                    if re.match(r'^[0-9a-fA-F]{2}$', hd):
                        result.append(chr(int(hd,16))); i += 4; continue
                result.append(s[i])
    ...
    def _extract_n_strings(self, source):
        ...
        start = source.index('{', m.start())
        depth = 0; end = -1; in_str = False; str_ch = None
        i = start
        while i < len(source):
            ch = source[i]
            if in_str:
        ...
        if end == -1: return None
        body = source[start+1:end]
        strings = []; current = []; in_str = False; str_ch = None; esc_next = False
        for ch in body:
            if esc_next:
            ...
            if in_str:
                ...
                current.append(ch)
                if ch == str_ch: strings.append(''.join(current)); current = []; in_str = False
                continue
            if ch in ('"', "'"): in_str = True; str_ch = ch; current.append(ch)
        result = []
        for s in strings:
            s = s.strip()
    ...
    def _extract_shuffle_pairs(self, source):
        ...
        for a_s, b_s in re.findall(r'\{(-?\d+(?:\s*[\+\-]\s*-?\d+)*)\s*[;,]\s*(-?\d+(?:\s*[\+\-]\s*-?\d+)*)\}', body):
            try:
                a = ast.literal_eval(a_s.replace('--','+').replace(' ',''))
                b = ast.literal_eval(b_s.replace('--','+').replace(' ',''))
    ...
    def _decode_b64(self, s, cmap):
        ...
        for ch in s:
            if ch == '=':
                if cnt == 3: buf.append((acc>>10)&0xFF); buf.append((acc>>2)&0xFF)
                elif cnt == 2: buf.append((acc>>4)&0xFF)
                break
            val = cmap.get(ch)
