...
class Lua51Parser:
    ...
    def _parse_header(self):
        if not (self.bc[:4]==b'\x1bLua' and self.bc[4]==0x51): raise ValueError("Not Lua 5.1")
...
class Lua51Decompiler:
    ...
    BINOP={12:'+',13:'-',14:'*',15:'/',16:'%',17:'^'}; UNOP={18:'-',19:'not ',20:'#'}
    ...
    def _func(self,func,name='f',is_main=False):
        ...
        if not is_main:
            self._emit(f'local function {name}({", ".join(params)})'); self.indent+=1
        regs={}; R=lambda r: regs.get(r,f'r{r}'); RK=lambda v: self._rk(v,consts,regs)
        i=0
        while i<len(code):
            ins=code[i]; op,a,b,c,bx,sbx=ins['op'],ins['a'],ins['b'],ins['c'],ins['bx'],ins['sbx']
            if op==0: regs[a]=R(b)
            elif op==1: regs[a]=self._fc(consts[bx] if bx<len(consts) else None)
            elif op==2: regs[a]='true' if b else 'false'; i+=1 if c else 0
            elif op==3:
            ...
            elif op==23: self._emit(f'if {RK(b)} {"==" if a==0 else "~="} {RK(c)} then'); self.indent+=1
            elif op==24: self._emit(f'if {RK(b)} {"<" if a==0 else ">="} {RK(c)} then'); self.indent+=1
            elif op==25: self._emit(f'if {RK(b)} {"<=" if a==0 else ">"} {RK(c)} then'); self.indent+=1
            elif op==26: self._emit(f'if {"not " if c==0 else ""}{R(a)} then'); self.indent+=1
            elif op==27: regs[a]=R(b); self._emit(f'if {"not " if c==0 else ""}{R(b)} then'); self.indent+=1
            elif op==28:
                fn=R(a); args='...' if b==0 else ('' if b==1 else ', '.join(R(a+k) for k in range(1,b)))
                call=f'{fn}({args})'
                if c==0: regs[a]=call
            ...
            elif op==31: self.indent=max(0,self.indent-1); self._emit('end')
            elif op==32:
                lv=f'i_{a}'; regs[a+3]=lv
                self._emit(f'for {lv} = {R(a)}, {R(a+1)}, {R(a+2)} do'); self.indent+=1
            elif op==33:
                vs=[self._t() for _ in range(c)]
                for k,v in enumerate(vs): regs[a+3+k]=v
                self._emit(f'for {", ".join(vs)} in {R(a)} do'); self.indent+=1
            elif op==34:
                ...
                pname=f'func_{bx}'
                if bx<len(protos): sv=self.indent; self._func(protos[bx],pname,False); self.indent=sv
                regs[a]=pname
            elif op==37:
                if b==0: regs[a]='...'
                else:
...
class WeAreDevsLifter(Transformer):
    ...
    def transform(self,code):
        ...
        try:
            ...
            if lifted and len(lifted)>20: return lifted
        except Exception as e: self.diagnostic=f"Lifter exception: {e}"
        return code
    def _try_lift(self,source):
        ...
        for chunk in decoded:
            if self._is_lua51_bytecode(chunk):
                res=self._deep_decompile(chunk)
                if res: return res
        full=b''.join(decoded)
        if self._is_lua51_bytecode(full):
            res=self._deep_decompile(full)
            if res: return res
        idx=full.find(b'\x1bLua')
        if idx!=-1 and idx+5<=len(full) and full[idx+4]==0x51:
            res=self._deep_decompile(full[idx:])
            if res: return res
        best=""
        for chunk in decoded:
            try:
                ...
                if len(text)>50 and printable/max(len(text),1)>0.8:
    ...
    def _deep_decompile(self, bc):
        try:
            parser=Lua51Parser(bc)
            func=parser.parse_function()
            srcs=[]
            def recurse(f):
                if f.get('source') and f['source'] and f['source'] not in ('@input','=stdin'):
                    try:
                        srcs.append(f['source'])
                    except: pass
                for p in f.get('protos',[]):
                    recurse(p)
            recurse(func)
            if srcs:
                return '\n'.join(srcs)
            ...
            self.diagnostic=f"Decompile error: {e}"
            return None
    ...
    def _build_char_map(self, source):
        ...
        assignments=[]; current=[]; paren_depth=0; in_string=False; string_char=None
        for ch in body:
            if in_string:
                current.append(ch)
                if ch==string_char: in_string=False
                continue
            if ch in ('"',"'"): in_string=True; string_char=ch; current.append(ch); continue
            if ch=='(': paren_depth+=1
    ...
    def _extract_n_strings(self, source):
        ...
        if not m: return None
        start=m.end()-1
        depth=0; end=-1; in_string=False; string_char=None
        for i in range(start,len(source)):
            ch=source[i]
            if in_string:
                if ch=='\\': i+=1; continue
                if ch==string_char: in_string=False
                continue
            if ch in ('"',"'"): in_string=True; string_char=ch; continue
            if ch=='{': depth+=1
            elif ch=='}':
                depth-=1
                if depth==0: end=i; break
        if end==-1: return None
        body=source[start+1:end]
        strings=[]; current=[]; in_string=False; string_char=None; escape_next=False
        for ch in body:
            if escape_next:
                current.append('\\'+ch); escape_next=False; continue
            if in_string:
                if ch=='\\': current.append(ch); escape_next=True; continue
                current.append(ch)
                if ch==string_char: strings.append(''.join(current)); current=[]; in_string=False
                continue
            if ch in ('"',"'"): in_string=True; string_char=ch; current.append(ch); continue
    ...
    def _extract_shuffle_pairs(self, source):
        ...
        pairs=[]
        for a_s,b_s in re.findall(r'\{(-?\d+(?:\s*[+\-]\s*-?\d+)*)\s*[;,]\s*(-?\d+(?:\s*[+\-]\s*-?\d+)*)\}',body):
            try:
                a=eval(a_s.replace('--','+').replace(' ',''))
                b=eval(b_s.replace('--','+').replace(' ',''))
                if a>0 and b>0: pairs.append((a,b))
            except: pass
        return pairs if len(pairs)==3 else None

    def _decode_b64(self, s, cmap):
        buf=bytearray(); acc=cnt=0
        for ch in s:
            if ch=='=':
