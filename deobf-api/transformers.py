    ...
    def transform(self, code):
        def try_decode(m):
            ...
            try:
                ...
                if len(text) > 30 and any(kw in text for kw in
                        ['function', 'local ', 'end', 'return', 'print(']):
                    return repr(text)
            except Exception:
                pass
            return m.group(0)
...
class Lua51Decompiler:
    ...
    def _func(self, func, name='f', is_main=False):
        ...
        while i < len(code):
            ins = code[i]
            op, a, b, c, bx, sbx = (ins['op'], ins['a'], ins['b'],
                                     ins['c'], ins['bx'], ins['sbx'])
            if   op == 0:  regs[a] = R(b)
                ...
                fn   = R(a)
                args = ('...' if b == 0 else
                        ('' if b == 1 else ', '.join(R(a + k) for k in range(1, b))))
                call = f'{fn}({args})'
                if   c == 0: regs[a] = call
...
class WeAreDevsLifter(Transformer):
    ...
    def _decode_b64(self, s, cmap):
        ...
        for ch in s:
            if ch == '=':
                if cnt == 3:
                    buf.append((acc >> 10) & 0xFF)
                    buf.append((acc >> 2)  & 0xFF)
                elif cnt == 2:
                    buf.append((acc >> 4)  & 0xFF)
                break
            val = cmap.get(ch)
            if val is None:
                continue
