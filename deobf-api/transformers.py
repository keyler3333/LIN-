...
class Lua51Parser:
    ...
    def _parse_header(self):
        if not (self.bc[:4] == b'\x1bLua' and self.bc[4] == 0x51): raise ValueError
...
class WeAreDevsLifter(Transformer):
    ...
    def _try_lift(self, source):
        ...
        for chunk in decoded:
            if self._is_lua51_bytecode(chunk): return self._decompile_bytecode(chunk)
        full = b''.join(decoded)
        if self._is_lua51_bytecode(full): return self._decompile_bytecode(full)
        idx = full.find(b'\x1bLua')
        if idx != -1 and idx+5 <= len(full) and full[idx+4] == 0x51: return self._decompile_bytecode(full[idx:])
        best = ""
        for chunk in decoded:
            try:
