...
class PlusEqualsTransformer(Transformer):
    _PAT = re.compile(r'^(\s*)(\w+)\s*=\s*\2\s*([\+\-\*\/])\s*(.+)$', re.MULTILINE)
    def transform(self, code):
        def _replace(m):
...
class Lua51Decompiler:
    ...
    def _fmt_const(c):
        ...
        return str(c)
    @staticmethod
    def _is_ident(s): return bool(re.match(r'^[A-Za-z_]\w*$', s))
...
class WeAreDevsLifter(Transformer):
    ...
    def _find_char_map(self, source):
        ...
        for m in re.finditer(r'\{([^{}]{200,})\}', source, re.DOTALL):
            ...
            for key, expr in re.findall(r'\[?"?([^"\]]+)"?\]?\s*=\s*(-?\d+(?:\s*[+\-]\s*\d+)*)', body):
                try: cmap[key.strip()] = eval(expr.replace(' ', '')) & 0x3F
                except: pass
    ...
    def _decode_custom_b64(self, s, char_map):
        ...
        for ch in s:
            if ch == '=':
                if count == 3: buf.extend([(acc>>16)&0xFF, (acc>>8)&0xFF])
