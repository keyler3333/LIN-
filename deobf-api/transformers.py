...
class Lua51Parser:
    ...
    def _int(self):
        data = self.bc[self.pos[0]:self.pos[0]+self.int_size]; self.pos[0] += self.int_size
        return int.from_bytes(data, 'little' if self.little_endian else 'big')
    def _sizet(self):
        data = self.bc[self.pos[0]:self.pos[0]+self.sizet_size]; self.pos[0] += self.sizet_size
        return int.from_bytes(data, 'little' if self.little_endian else 'big')
    def _double(self):
    ...
    def _func(self, func, name='f', is_main=False):
        ...
        while i < len(code):
            ...
            if op == 0: regs[a] = R(b)
                ...
                self._emit(f'if {RK(b)} {"==" if a == 0 else "~="} {RK(c)} then'); self.indent += 1
            elif op == 24:
                self._emit(f'if {RK(b)} {"<" if a == 0 else ">="} {RK(c)} then'); self.indent += 1
            elif op == 25:
                self._emit(f'if {RK(b)} {"<=" if a == 0 else ">"} {RK(c)} then'); self.indent += 1
            elif op == 26:
                self._emit(f'if {"not " if c == 0 else ""}{R(a)} then'); self.indent += 1
            elif op == 27:
                regs[a] = R(b); self._emit(f'if {"not " if c == 0 else ""}{R(b)} then'); self.indent += 1
            elif op == 28:
                fn = R(a)
...
class WeAreDevsLifter(Transformer):
    ...
    @staticmethod
    def _is_lua51_bytecode(data): return len(data) >= 12 and data[:4] == b'\x1bLua' and data[4] == 0x51
